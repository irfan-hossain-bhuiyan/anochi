...
pub struct TypeDefinition(CompTimeTypeGeneric<Self>);

impl TypeDefinition {
    /// Get the inner TypeGeneric
    pub fn inner(&self) -> &CompTimeTypeGeneric<TypeDefinition> {
        &self.0
    }

    /// Get builtin kind if this is a builtin type
    pub fn as_builtin(&self) -> Option<CompTimeBuiltinType> {
        match &self.0 {
            CompTimeTypeGeneric::Builtin(kind) => Some(*kind),
            _ => None,
        }
    }
    ...
    pub fn as_product(&self) -> Option<&BTreeMap<Identifier, TypeDefinition>> {
        match &self.0 {
            CompTimeTypeGeneric::Product(fields) => Some(fields),
            _ => None,
        }
    }
    ...
    pub fn as_sum(&self) -> Option<&BTreeSet<TypeDefinition>> {
        match &self.0 {
            CompTimeTypeGeneric::Sum(variants) => Some(variants),
            _ => None,
        }
    }
    ...
    pub fn as_reference(&self) -> Option<&TypeDefinition> {
        match &self.0 {
            CompTimeTypeGeneric::Reference(inner) => Some(inner),
            _ => None,
        }
    }
}
...
impl TypeDefinition {
    ...
    pub fn into_optimized(self,container: &mut TypeContainer)->OptimizedTypeDefinition{
       let output=self.0.inner_map(&mut |x|container.store_type_def(x));
       OptimizedTypeDefinition(output)
    }
}
