use crate::{
    ...
    types::{BuiltinKind, TypeContainer, TypeDefinition, UnifiedTypeDefinition}, vm::tree_walk::VmValue,
};
...
fn test_vm_value_for_complex_type(){
    ...
    let type_def=container.get_type(&id1).unwrap().clone().into_type_def(&container);
    let mut map=BTreeMap::new();
    map.insert(Identifier::new("x"), TypeDefinition::builtin(BuiltinKind::I64));
    map.insert(Identifier::new("y"), TypeDefinition::builtin(BuiltinKind::I64));
