    ...
    match expression {
        Expression::Literal(literal) => match literal {
            Literal::Identifier(name) => {
                let var_state = vm
                    .variables
                    .get_variable_data(name)
                    .ok_or_else(|| map_err(VmErrorType::UndefinedIdentifier(name.clone())))?;
                Ok(var_state.type_id)
            }
            Literal::Bool(_) => {
                let type_def =
                    UnifiedTypeDefinition::builtin(crate::types::CompTimeBuiltinType::Bool);
                Ok(vm.types.store_unified_type(type_def))
            }
            Literal::Integer(_) => {
                let type_def =
                    UnifiedTypeDefinition::builtin(crate::types::CompTimeBuiltinType::Int);
                Ok(vm.types.store_unified_type(type_def))
            }
            Literal::Float(_) => {
                let type_def =
                    UnifiedTypeDefinition::builtin(crate::types::CompTimeBuiltinType::Float);
                Ok(vm.types.store_unified_type(type_def))
            }
            Literal::String(_) => {
                unimplemented!()
        ...
        Expression::Unary { operator, operand } => match operator {
            UnaryOperator::Ref => {
                ...
                let type_def =
                    UnifiedTypeDefinition::reference(UnifiedTypeDefinition::TypeId(operand_type));
                Ok(vm.types.store_unified_type(type_def))
            }
            UnaryOperator::Deref => {
