use crate::{ast::Identifier, types::TypeContainer, vm::tree_walk::{ScopeStack, vm_value::{ParsedValueType, ValuePrimitive}, VmErrorType}};
use num_bigint::BigInt;
    ...
    fn test_scope_variable_access_across_scopes() {
        ...
        let value = ValuePrimitive::Integer(BigInt::from(42)).into_vm_value_generalized(&mut type_container);
    ...
    fn test_mutable_variable_access_and_modification() {
        ...
        let initial_value = ValuePrimitive::Integer(BigInt::from(10)).into_vm_value_generalized(&mut type_container);
        let modified_value = ValuePrimitive::Integer(BigInt::from(20)).into_vm_value_generalized(&mut type_container);

        stack.insert_variable_default(var_name.clone(), initial_value.clone());
        
        stack.create_scope();
        
        stack.set_value_from_name(&var_name, modified_value.clone()).unwrap();
        
        let retrieved = stack.get_value_from_name(&var_name, &type_container).unwrap();
    ...
    fn test_undefined_variable_mutable_access() {
        ...
        let mut type_container = TypeContainer::default();
        let nonexistent_var = Identifier::new("nonexistent".to_string());
        let dummy_value = ValuePrimitive::Integer(BigInt::from(0)).into_vm_value_generalized(&mut type_container);
    ...
    fn test_has_variable_across_scopes() {
        ...
        let value = ValuePrimitive::Integer(BigInt::from(1)).into_vm_value_generalized(&mut type_container);
    ...
    fn test_nested_scopes_complex() {
        ...
        let global_value = ValuePrimitive::Integer(BigInt::from(100)).into_vm_value_generalized(&mut type_container);
        let scope1_value = ValuePrimitive::Integer(BigInt::from(200)).into_vm_value_generalized(&mut type_container);
        let scope2_value = ValuePrimitive::Integer(BigInt::from(300)).into_vm_value_generalized(&mut type_container);
